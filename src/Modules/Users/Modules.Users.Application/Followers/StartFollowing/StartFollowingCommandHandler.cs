...
internal sealed class StartFollowingCommandHandler(
    ...
    IUserContext userContext) : ICommandHandler<StartFollowingCommand>
{
    public async Task<Result> Handle(StartFollowingCommand command, CancellationToken cancellationToken)
    {
        ...
        Result<Follower> result = await followerService.StartFollowingAsync(user, followed, cancellationToken);
        if (result.IsFailure)
        {
            return result;
        }
